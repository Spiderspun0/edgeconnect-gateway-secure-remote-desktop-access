import React from 'react';
import { Server, Globe, Terminal, ChevronRight, Settings, Loader2 } from 'lucide-react';
import { Card, CardContent, CardHeader, CardFooter } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import type { RemoteMachine } from '@shared/types';
import { useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';
interface MachineCardProps {
  machine: RemoteMachine;
  onConnect?: () => void;
  isConnecting?: boolean;
}
export function MachineCard({ machine, onConnect, isConnecting }: MachineCardProps) {
  const navigate = useNavigate();
  const isActuallyConnecting = isConnecting || machine.status === 'connecting';
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      whileHover={{ y: -4 }}
      transition={{ duration: 0.3 }}
    >
      <Card className="glass-card group transition-all duration-300 hover:shadow-xl overflow-hidden border-border/50">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg bg-cf-blue-500/10 text-cf-blue-600">
              <Server className="w-5 h-5" />
            </div>
            <div>
              <h3 className="font-semibold text-lg leading-none group-hover:text-cf-blue-600 transition-colors">
                {machine.name}
              </h3>
              <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                <Globe className="w-3 h-3" /> {machine.host}:{machine.port}
              </p>
            </div>
          </div>
          <Badge
            variant={machine.status === 'online' ? 'default' : 'secondary'}
            className={
              machine.status === 'online' 
                ? 'bg-green-500/10 text-green-600 border-green-500/20 hover:bg-green-500/20' 
                : machine.status === 'connecting'
                ? 'bg-amber-500/10 text-amber-600 border-amber-500/20'
                : ''
            }
          >
            {machine.status}
          </Badge>
        </CardHeader>
        <CardContent className="pt-4">
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            <div className="flex items-center gap-1">
              <Terminal className="w-3 h-3" />
              <span>{machine.authMethod === 'token' ? 'Encrypted Tunnel' : 'SSH/Password'}</span>
            </div>
          </div>
        </CardContent>
        <CardFooter className="flex gap-2">
          <Button
            variant="gradient"
            className="flex-1"
            onClick={onConnect}
            disabled={isActuallyConnecting || machine.status === 'offline'}
          >
            {isActuallyConnecting ? (
              <Loader2 className="w-4 h-4 animate-spin" />
            ) : (
              <>
                Connect
                <ChevronRight className="ml-1 w-4 h-4" />
              </>
            )}
          </Button>
          <Button
            variant="outline"
            size="icon"
            onClick={() => navigate(`/machines/${machine.id}/edit`)}
            title="Manage machine"
          >
            <Settings className="w-4 h-4" />
          </Button>
        </CardFooter>
      </Card>
    </motion.div>
  );
}